...
- **Catch test:** for each `retrieval_load_ch<N>_cards.json`, every note's Text must exist in `chapter_<N>_cards.json` AND in that chapter's live deck (`Book Highlights` since 2026-08-24; `claude review` before it). Run it after any write.
