...
26. **A procedure is carded as DECISIONS, never as a recitation of its steps.**
    When the source teaches a skill drill, protocol or algorithm, the card asks *"given this state of the world, what now?"* — not *"list the N steps."* Full playbook and evidence: `card-recipes.md` §12.
    - **Preferred shapes, in order:** a **decision-point vignette** (the discriminating state visible in the stem, one blank on the action); a **decision table** (condition → action rows under one cloze number, each row cued by its own condition, never by its position); the **decidable residue** of a physical skill (indication, landmark, contraindication, complication, the step people get wrong); and a **forward chain** only when order genuinely is the knowledge and is not implied by the conditions.
    - **Never card the motion of a psychomotor skill.** Measured across Parker's 85,212-note collection — including 10,907 anesthesia cards — `"place your hands"`, `"position your"`, `"how do you perform"` and `"steps to perform"` return **zero hits**. Professionals train physical skill by doing it and card only what a decision hangs on.
    - **Attach the procedure's own plate to the back**, so the whole sequence stays visible after he answers.
    *Evidence that created this rule (2026-08-03):* Parker asked how to card systematic skill-based material without producing junk, and pointed at his own AnKing decks as the authority. The measurement: "next step in management" cards are **93% one span, 0% ≥5 spans, 95% question-form**; the flagship AnKing Step Deck is **89% single-span**; and the whole collection contains essentially no psychomotor-technique cards. See `card-recipes.md` §12 for the tables.
