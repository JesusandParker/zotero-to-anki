...
23. **A grouped reveal must be ONE retrieval — chunk the set, don't flatten it.**
    ...
    - **A per-item cue lowers the load and is the cheapest fix available.** `Neonate — {{c1::100 to 180}}` and `Transports oxygen → {{c1::red blood cells}}` are N small independently-cued retrievals, not one N-wide recall, so a match/classify card or a keyed panel is exempt at any length. Converting a bare list into a keyed one is often better than splitting it.
