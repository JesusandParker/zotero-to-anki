...
sat in Back Extra, leaving its meaning card unanswerable before he could read the script, the 2026-09-03 card
that played an Egyptian (maSri) pronunciation he is never tested on, and the 2026-09-11
letter-form note with all four positional shapes crammed under one cloze number. The suite fails if the checker passes any BAD case
...
if rc == 0:
    ...
    for must in ("U1-lrm", "U3-no-dup-audio", "U4-no-boilerplate", "U2-media-lower",
                 "V1-two-way", "V3-translit-with-arabic", "V4-no-excluded-dialect-audio", "C1-property", "C2-two-way", "C3-roster", "C4-membership",
                 "F1-position-cued", "F2-two-way", "F3-one-shape-per-note", "F4-plate"):
        if must not in out:
